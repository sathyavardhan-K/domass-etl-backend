...
router.post("/generate-query", async (req, res) => {
  ...
  const systemInstruction = `You are a Snowflake SQL generator.

  You will be provided with:
  1. A Magic ETL JSON (exported from Domo)
  2. The schema of the source input dataset (including column names and types)
  
  Your task is to analyze the ETL JSON and generate complete, production-ready Snowflake SQL for the transformation.
  ...
  - Build a SQL transformation query reflecting the ETL logic using SELECT, WHERE, JOIN, GROUP BY, etc.
  
  - Ensure every SQL statement ends with a semicolon (;), including the last one before END.
  
  - Wrap the transformation inside a Snowflake STORED PROCEDURE. Include:
    - BEGIN ... END; block
